-- =============================================
-- COMPREHENSIVE SECURITY OVERHAUL (SAFE VERSION)
-- =============================================
...
-- 4. STORAGE SECURITY - MAKE FOOD-IMAGES PRIVATE AND SECURE POLICIES
-- =============================================
UPDATE storage.buckets 
SET public = false 
WHERE id = 'food-images';

-- Drop all existing storage policies for food-images bucket
...
DROP POLICY IF EXISTS "Users can view food images" ON storage.objects;
...
DROP TRIGGER IF EXISTS food_listings_audit_trigger ON public.food_listings;
CREATE TRIGGER food_listings_audit_trigger
  AFTER INSERT OR UPDATE OR DELETE ON public.food_listings
  FOR EACH ROW EXECUTE FUNCTION public.audit_trigger_func();

DROP TRIGGER IF EXISTS claims_audit_trigger ON public.claims;
...
CREATE INDEX IF NOT EXISTS idx_signup_attempts_ip_time ON public.signup_attempts (ip_address, attempt_time);
CREATE INDEX IF NOT EXISTS idx_signup_attempts_email_time ON public.signup_attempts (email, attempt_time);